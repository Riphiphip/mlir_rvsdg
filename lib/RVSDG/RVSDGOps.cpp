 ...
 * Load
 */
LogicalResult rvsdg::Load::verify() {
  ...
  if (auto rvsdgPtrType = this->getPointer().getType().dyn_cast_or_null<RVSDGPointerType>()) {
...
* Store
*/
LogicalResult rvsdg::Store::verify() {
  if (auto llvmPtr = this->getPointer().getType().dyn_cast<mlir::LLVM::LLVMPointerType>()) {
    if (llvmPtr.isOpaque()) {
      return LogicalResult::success();
    }
 ...
 * Alloca
 */
LogicalResult rvsdg::Alloca::verify() {
  auto outputType = this->getOutput().getType();
